...
void setup() {  
  ...
  while(true){
   if(Serial1.available()>0){                                // aquire GPS signal
 ...
 setup_mpu_6050_registers();                                //IMU Calibration
 calibrate_acc_gyros();
 Serial.println(F("finished calabrating acc"));     
 digitalWrite(49, HIGH);
 delay(20000);                                              //LED comes on for 20 seconds (GPS Ready)
 digitalWrite(49, LOW);
...
void loop(){                                               //MAIN LOOP
  ...
  if(MIC_OFF==0){                                          //MIC ON or OFF
    MIC_opperate(time1);                                   //Record MIC
  }
  
  time1=update_time();//update time again

  
  if(ACC_OFF==0){                                          //ACC ON or OFF 
    IMU_delay=millis();
    while(IMU_delay==0||millis()-IMU_delay<=IMU_TIME){     //Record IMU
...
void read_mpu_6050_data(){                                             //Subroutine for reading the raw gyro and accelerometer data
  Wire.beginTransmission(0x68);                                        //Start communicating with the IMU
  ...
  Wire.read()<<8|Wire.read();                                          //Add the low and high byte to the temperature variable(unused)
...
void setup_mpu_6050_registers(){
  ...
  Wire.beginTransmission(0x68);                                        //Start communication MPU-6050 register
  Wire.write(0x6B);                                                    //Send the starting register
  Wire.write(0x00);                                                    //Set the starting register
  Wire.endTransmission();                                              //End the transmission
  //Configure the accelerometer (+/-2g)
  Wire.beginTransmission(0x68);                                        //Start communicating with the MPU-6050
  Wire.write(0x1C);                                                    //Send starting register
  Wire.write(0x00);                                                    //Set starting register
  Wire.endTransmission();                                              //End the transmission
  //Configure the gyro (500dps full scale)
  Wire.beginTransmission(0x68);                                        //Start communicating with the MPU-6050
  Wire.write(0x1B);                                                    //Send starting register
  Wire.write(0x08);                                                    //Set starting register
  Wire.endTransmission();                                              //End the transmission
}
...
void calibrate_acc_gyros(){
  for (int cal_int = 0; cal_int < 2000 ; cal_int ++){                  //Run this code 2000 times
    read_mpu_6050_data();                                              //Read the raw acc and gyro data from the MPU-6050
    gyro_x_cal += gyro_x;                                              //Add each time the gyro x-axis offset to the gyro_x_cal variable
    gyro_y_cal += gyro_y;                                              //Add each time the gyro y-axis offset to the gyro_y_cal variable
    gyro_z_cal += gyro_z;                                              //Add each time the gyro z-axis offset to the gyro_z_cal variable
  ...
  gyro_x_cal /= 2000;                                                  //Average gyro_x_cal variable over 2000 samples to get the offset
  gyro_y_cal /= 2000;                                                  //Average gyro_y_cal variable over 2000 samples to get the offset
  gyro_z_cal /= 2000;                                                  //Average gyro_z_cal variable over 2000 samples to get the offset
